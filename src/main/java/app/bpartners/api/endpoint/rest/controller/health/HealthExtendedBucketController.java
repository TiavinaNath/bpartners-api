...
@RestController
@AllArgsConstructor
public class HealthExtendedBucketController {

  ExtendedBucketComponent extendedBucketComponent;

  private static final String HEALTH_KEY = "health/";

  @GetMapping(value = "/health/extended-bucket")
  ...
  private File can_upload_file_then_download_file(File toUpload, String bucketKey)
      throws IOException {
    extendedBucketComponent.upload(toUpload, bucketKey, true);

    var downloaded = extendedBucketComponent.download(bucketKey, true);
  ...
  private FileHash can_upload_directory(File toUpload, String bucketKey) {
    var hash = extendedBucketComponent.upload(toUpload, bucketKey, true);
  ...
  private URL can_presign(String fileBucketKey) {
    return extendedBucketComponent.presign(fileBucketKey, Duration.ofMinutes(2));
  }
}
